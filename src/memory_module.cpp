...
namespace HammerIdFix::shared::modules
{
    void Initialize()
    {
        if (!shared::modules::moduleList.empty()) return;
    ...
    for (auto entry = peb->Ldr->InMemoryOrderModuleList.Flink; entry != &peb->Ldr->InMemoryOrderModuleList; entry = entry->Flink)
    {
        ...
        dl_iterate_phdr([](struct dl_phdr_info* info, size_t, void*)
        {
            ...
            shared::modules::moduleList.emplace_back(std::move(mod));
            return 0;
        }, nullptr);
#endif
    }
    ...
    std::replace(name.begin(), name.end(), '\\', '/');
        ...
        const auto it = std::find_if(shared::modules::moduleList.begin(), shared::modules::moduleList.end(), [&name](const std::unique_ptr<CModule>& i)
        {
            return !i->m_pszModule.empty() && name.size() >= i->m_pszModule.size() &&
                name.substr(name.size() - i->m_pszModule.size()) == i->m_pszModule;
        });

        if (it == shared::modules::moduleList.end())
        {
            return nullptr;
        }
    ...
    constexpr std::array<const char*, 2> modules_to_read_from_disk = {
    ...
    int GetModuleInformation(HINSTANCE hModule, void** base, size_t* length, std::vector<Section>& m_sections)
    {
        ...
        if (fstat(fd, &st) == 0)
        {
            ...
            if (map != MAP_FAILED)
            {
                ...
                const char* strTab = reinterpret_cast<const char*>(reinterpret_cast<uintptr_t>(ehdr) + shdrs[ehdr->
                    e_shstrndx].sh_offset);

                for (auto i = 0; i < ehdr->e_phnum; ++i)
                {
                    ElfW(Phdr)* phdr = reinterpret_cast<ElfW(Phdr)*>(reinterpret_cast<uintptr_t>(ehdr) + ehdr->e_phoff +
                        i * ehdr->e_phentsize);
                ...
                {
                    ElfW(Shdr)* shdr = reinterpret_cast<ElfW(Shdr)*>(reinterpret_cast<uintptr_t>(shdrs) + i * ehdr->
                        e_shentsize);
    ...
    void* CModule::FindInterface(std::string_view name)
    {
        if (_interfaces.empty())
        {
            ...
            for (auto list = interface_reg; list != nullptr; list = list->pNext)
            {
                ...
                if (const std::string_view interface_name = list->szName; interface_name == name) ret_interface =
                    interface_addrss;
